...
<div class="post-container">
    ...
    @php
        $isCurrentUser = auth()->check() && auth()->user()->id === $post->user->id;
        $profileImageUrl = $isCurrentUser ? auth()->user()->profile_image_url : $post->user->profile_image_url;
    @endphp
    <a href="{{ route('profile.show', $post->user) }}">
    <img src="{{ asset($profileImageUrl ? 'storage/' . $profileImageUrl : 'profile.png') }}" alt="Profile Image" class="profile">
    </a>
<strong class="nav-link" >{{ $post->user->username }} | {{ $post->user->course }}</strong>
</div>
...
<div id="comment-box-{{ $post->id }}" class="comment-box mt-3">
    ...
    <img src="{{ asset('storage/' . (auth()->user()->profile_image_url ?? 'default-profile.png')) }}"  alt="Profile Image" class="profile-image2">
    ...
    <form id="expanded-comment-box-{{ $post->id }}" action="{{ route('posts.comment', $post) }}" method="POST" class="expanded-comment-box" style="display: none;">
        @csrf
        <img src="{{ asset('storage/' . (auth()->user()->profile_image_url ?? 'default-profile.png')) }}"  alt="Profile Image" class="profile-image">
